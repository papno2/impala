...
namespace impala {
  ...
  SortedRunMerger::SortedRunWrapper::SortedRunWrapper(SortedRunMerger* parent, const RunBatchSupplierFn& sorted_run)
...
Status SortedRunMerger::Heapify(int parent_index) {
  const HeapifyHelperFn heapify_helper_fn = codegend_heapify_helper_fn_.load();

  if (heapify_helper_fn != nullptr) {
    RETURN_IF_ERROR(heapify_helper_fn(this, parent_index));
  } else {
    RETURN_IF_ERROR(HeapifyHelper(parent_index));
  }
  return Status::OK();
}
...
Status SortedRunMerger::Codegen(FragmentState* state, llvm::Function* compare_fn,
  ...
  llvm::Function* fn = codegen->GetFunction(IRFunction::SORTED_RUN_MERGER_HEAPIFY_HELPER, true);
  DCHECK(fn != NULL);
