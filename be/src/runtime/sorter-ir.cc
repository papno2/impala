...
//////////////////////Beginning of side 3way quicksort with compare////////////////////////


Status IR_ALWAYS_INLINE Sorter::TupleSorter::PartitionCompare(TupleIterator begin,
    TupleIterator end, const Tuple* pivot, TupleIterator* cut_left, TupleIterator* cut_right) {
  // Hoist member variable lookups out of loop to avoid extra loads inside loop.
  Run* run = run_;
  int tuple_size = tuple_size_;
...
Status Sorter::TupleSorter::SortHelperCompare(TupleIterator begin, TupleIterator end) {
  // Use insertion sort for smaller sequences.
  while (end.index() - begin.index() > INSERTION_THRESHOLD) {
    // Select a pivot and call Partition() to split the tuples in [begin, end) into three
    // groups (< pivot, == pivot, and > pivot) in-place. 'cut_left' and 'cut_right' are
    // the indices of the first tuple in the second and first tuple in the third group.
    // The second group contains the equal elements, thus already sorted.
    Tuple* pivot = SelectPivot9(begin, end);
    TupleIterator cut_left;
    TupleIterator cut_right;
    RETURN_IF_ERROR(PartitionCompare(begin, end, pivot, &cut_left, &cut_right));

    // Recurse on the smaller partition. This limits stack size to log(n) stack frames.
    if (cut_left.index() - begin.index() < end.index() - cut_right.index()) {
      // Left partition is smaller.
      RETURN_IF_ERROR(SortHelperCompare(begin, cut_left));
      begin = cut_right;
    } else {
      // Right partition is equal or smaller.
      RETURN_IF_ERROR(SortHelperCompare(cut_right, end));
      end = cut_left;
    }
  }
...
///////////////////Beginning of adaptive quicksort with less and equal///////////////////


Status Sorter::TupleSorter::AdaptiveSortHelper(TupleIterator begin, TupleIterator end) {
  //if (CheckSorted(begin, end)) return Status::OK();
  // Use insertion sort for smaller sequences.
  while (end.index() - begin.index() > INSERTION_THRESHOLD) {
    // Select a pivot and call Partition() to split the tuples in [begin, end) into three
    ...
    TupleRow* tr1 = reinterpret_cast<TupleRow*>(&tuples[0]);
    TupleRow* tr2 = reinterpret_cast<TupleRow*>(&tuples[1]);
    TupleRow* tr3 = reinterpret_cast<TupleRow*>(&tuples[2]);

    int t1_lt_t2 = Compare(tr1, tr2);
    int t2_lt_t3 = Compare(tr2, tr3);
    int t1_lt_t3 = Compare(tr1, tr3);
    Tuple* pivot;
    ...
    if (t1_lt_t2*t1_lt_t3*t2_lt_t3 == 0) {
      ...
      if (cut_left.index() - begin.index() < end.index() - cut_right.index()) {
        // Left partition is smaller.
        RETURN_IF_ERROR(AdaptiveSortHelper(begin, cut_left));
        begin = cut_right;
      } else {
        // Right partition is equal or smaller.
        RETURN_IF_ERROR(AdaptiveSortHelper(cut_right, end));
        end = cut_left;
      }
    } else {
      TupleIterator cut;
      ...
      if (cut.index() - begin.index() < end.index() - cut.index()) {
        // Left partition is smaller.
        RETURN_IF_ERROR(AdaptiveSortHelper(begin, cut));
        begin = cut;
      } else {
        // Right partition is equal or smaller.
        RETURN_IF_ERROR(AdaptiveSortHelper(cut, end));
        end = cut;
      }
    }
  }
...
Status Sorter::TupleSorter::StandardSortHelper(TupleIterator begin, TupleIterator end) {
  ...
  while (end.index() - begin.index() > INSERTION_THRESHOLD) {
    ...
    // the second group.
    Tuple* pivot = (SELECTION_THRESHOLD > (end.index() - begin.index())) ? SelectPivot(begin, end) : SelectPivot9(begin,end);
    TupleIterator cut;
    RETURN_IF_ERROR(StandardPartition(begin, end, pivot, &cut));
...
Tuple* IR_ALWAYS_INLINE Sorter::TupleSorter::SelectPivot9(
  ...
  return MedianOfThree(MedianOfThree(tuples[0], tuples[1], tuples[2]), MedianOfThree(tuples[3], tuples[4], tuples[5]), MedianOfThree(tuples[6], tuples[7], tuples[8]));
}
